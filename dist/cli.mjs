#!/usr/bin/env node
import { constants as constants$5 } from 'node:os';
import N$1 from 'tty';
import { transformSync } from 'esbuild';
import { v as version } from './package-CG_q1brP.mjs';
import { r as require, g as getPipePath, i as isWindows$3 } from './get-pipe-path-D2pYDmQS.mjs';
import { pathToFileURL, fileURLToPath } from 'node:url';
import require$$0$3 from 'child_process';
import require$$0$2 from 'path';
import require$$0$1 from 'fs';
import { i as isFeatureSupported, m as moduleRegister, t as testRunnerGlob } from './node-features-Bye2pwSD.mjs';
...
const createIpcServer = async () => {
  ...
  await fs$5.promises.mkdir(tmpdir, { recursive: true });
...
const watchCommand = G({
  ...
  const relaySignal = (signal) => {
    ...
    if (runProcess?.exitCode === null) {
      ...
      process.exit(constants$5.signals[signal]);
    }
  };
    ...
    if (signalFromChild !== signal) {
      ...
      if (isChildResponsive !== signal) {
        childProcess.on("exit", () => {
          const exitCode = constants$5.signals[signal];
...
`);
  ...
  if (isFeatureSupported(testRunnerGlob) && interceptedFlags.test && firstArgs.length === 0) {
  ...
  childProcess.on(
    ...
    (exitCode) => {
      if (exitCode === null) {
        exitCode = constants$5.signals[childProcess.signalCode] + 128;
      }
